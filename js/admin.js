$(document).ready(function () {
    $(document).on("click", ".btnModifier", function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $("#add-scenario-modal .thumb").html('<img src=".' + response.image + '">');
                $("#add-scenario-modal input[name=short_resume]").val(response.short_resume);
                $("#add-scenario-modal textarea[name=resume_complet]").val(response.resume_complet);
                $("#add-scenario-modal select[name=difficulty]").val(response.difficulty);
                $("#add-scenario-modal select[name=min_players]").val(response.min_players);
                $("#add-scenario-modal select[name=max_players]").val(response.max_players);
    ...
    $(".add-scenario-form").submit(function (e) {
        ...
        $.ajax({
            url: "ajax.php?action=" + (
            action == "add" ? "add_scenario" : "update_scenario"
        ),
            type: "POST",
            data: formData,
            dataType: "json",
